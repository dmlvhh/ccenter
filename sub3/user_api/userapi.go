...
import (
	...
	"go-zero2/sub3/user_api/internal/config"
	"go-zero2/sub3/user_api/internal/handler"
...
func registerToEtcd(c config.Config) error {
	...
	go func() {
		for {
			_, err := cli.KeepAliveOnce(context.Background(), leaseResp.ID)
			if err != nil {
				logx.Errorf("续期失败: %v", err)
				return
			}
